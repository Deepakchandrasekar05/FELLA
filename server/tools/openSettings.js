// server/tools/openSettings.js — Open Windows/macOS/Linux system settings
import { execa } from 'execa';

const WINDOWS_SETTINGS_MAP = {
...
function resolveWindowsSetting(setting) {
...
export function extractSettingRequest(userMessage) {
...
async function openWindowsSetting(setting) {
...
async function openMacSetting(setting) {
  const key = normalizeSetting(setting);
  const paneMap = {
...
async function openLinuxSetting(setting) {
  const key = normalizeSetting(setting);
  const gnomeMap = {
    wifi: ['wifi'],
    network: ['network'],
...
export async function openSettings(args) {
