// server/tools/browserAutomation.js - Playwright-backed browser control via Chrome CDP
...
function asPositiveNumberString(value) {
...
function parseCommandFallback(args) {
...
function extractOpenSiteIntent(goal) {
  const trimmed = goal.trim();
  const openMatch = trimmed.match(/^(?:open|go\s+to|navigate\s+to|launch)\s+(.+?)(?:\s+in\s+(?:chrome|browser|edge|firefox))?$/i);
  if (!openMatch?.[1]) return null;

  const wantsNewTab = /\bnew\s+tab\b/i.test(trimmed);

  const rawTarget = openMatch[1]
    .replace(/\s+in\s+(?:(?:a|the)\s+)?new\s+tab(?:\s+in\s+(?:chrome|browser|edge|firefox|safari))?$/i, ' ')
    .replace(/\s+in\s+(?:chrome|browser|edge|firefox|safari)$/i, ' ')
    .replace(/\b(?:website|site|web)\b/gi, ' ')
    .replace(/\s+/g, ' ')
    .trim();
  if (!rawTarget) return null;

  const url = normaliseWebsiteTarget(rawTarget);
  if (!url) return null;

  const isWebLike = /^https?:\/\//i.test(url) || /\.[a-z]{2,}(?:[/:?#]|$)/i.test(url);
  if (!isWebLike) return null;
...
async function readBodyText(currentPage) {
  const body = currentPage.locator('body');
  await body.first().waitFor({ state: 'attached', timeout: 8000 });
  return body.innerText();
}

async function safeGetPageTitle(currentPage) {
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      await currentPage.waitForLoadState('domcontentloaded', { timeout: 3000 });
...
function extractWeeklyDownloadsFromText(text) {
  const compact = text.replace(/\s+/g, ' ').trim();
  const match = compact.match(/weekly downloads\s*([\d,]+(?:\.\d+)?[kKmM]?)/i);
  return match?.[1] ?? null;
}

async function extractNpmWeeklyDownloads(currentPage) {
  try {
...
async function focusGoogleDocsEditor(currentPage) {
...
async function appendToGoogleDoc(currentPage, text) {
...
async function setGoogleDocsFont(currentPage, font) {
  await ensureGoogleDocPage(currentPage);
  await focusGoogleDocsEditor(currentPage);
...
async function setGoogleDocsFontSize(currentPage, size) {
  await ensureGoogleDocPage(currentPage);
  await focusGoogleDocsEditor(currentPage);
...
async function setGoogleDocsTextColor(currentPage, color) {
  await ensureGoogleDocPage(currentPage);
  await focusGoogleDocsEditor(currentPage);
...
async function addGoogleDocsHeaderFooter(currentPage, headerText, footerText) {
  await ensureGoogleDocPage(currentPage);
...
export async function browserAutomation(args) {
  const normalizedArgs = parseCommandFallback(args);
  const action = asString(normalizedArgs['action']).toLowerCase();
  ...
  switch (action) {
    case 'navigate': {
      ...
      const settledUrl = currentPage.url();
      const title = await safeGetPageTitle(currentPage);
      return `Navigated to ${settledUrl} - ${title}`;
    }
...
export async function agenticBrowserTask(goal, onStep) {
  ...
  const openSiteIntent = extractOpenSiteIntent(trimmedGoal);
  if (openSiteIntent) {
    return browserAutomation({
      action: 'navigate',
      url: openSiteIntent.url,
      newTab: openSiteIntent.newTab,
    });
  }
