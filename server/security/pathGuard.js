// server/security/pathGuard.js - Cross-platform path resolution and safety policy
import { homedir, tmpdir } from 'node:os';
import { resolve, isAbsolute, join, normalize } from 'node:path';
import { execSync } from 'node:child_process';

function winShellFolder(name) {
...
function assertAllowedWindows(absPath) {
  const blocked = [
    'C:\\Windows',
    'C:\\Program Files',
    'C:\\Program Files (x86)',
...
function assertAllowedMac(absPath) {
...
function assertAllowedLinux(absPath) {
...
function remapWindowsKnownFolderPath(rawPath) {
...
export function assertAllowed(absPath) {
  if (process.platform === 'win32') {
